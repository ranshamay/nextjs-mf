                  ...
                  Template.indent([
                    ...
                    */
                    ...
                    requestedRemote = await requestedRemote()
                        ...
                        Template.indent([
                          this._getLogger(`'load script callback fired ' + scriptUrl`),
                          "if(err) {console.error('error loading remote chunk', scriptUrl.toString(),'got',content); return reject(err);}",
                          'var chunk = {};',
                          "if(typeof process !== 'undefined') {",
                          'try {',
                          this._getLogger(`'evaluating content in vm' + scriptUrl`),
                          "require('vm').runInThisContext('(function(exports, require, __dirname, __filename) {' + content + '\\n})', filename)" +
                          "(chunk, require, require('path').dirname(filename), filename);",
                          '} catch (e) {',
                          ...
                          '}',
                          '} else {',
                          this._getLogger(`'evaluating content2' + chunkName`),
                          "eval('(function(exports, require, __dirname, __filename) {' + content + '\\n})')(chunk, __webpack_require__, '.', chunkName);",
                          '}',
                          this._getLogger(`'evaluating content DONE + chunkName'`),
                          'installChunk(chunk);',
                        ]),
                        '});',
                      ]),
